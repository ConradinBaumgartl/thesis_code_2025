...
hist_csv <- paste0(args[2], args[4], ".SINGLE.HIST", ".csv")
curve_csv <- paste0(args[2], args[4], ".SINGLE.CURVE", ".csv")

fam <- args[3]
gen <- args[4]

total_sequences <- readDNAStringSet(args[1])

# Calculation

# what dinucleotides
dinucleotides <- list("YY", "SS", "WW", "GG", "AA", "TT", "CC", "RR")


# calculate the histogram
df <- tibble()
hist_master <- tibble()
for(name in names(total_sequences)){
  
  # add the reverse complement
  # curr_sequence <- DNAStringSet(list(total_sequences[[name]], Biostrings::reverseComplement(total_sequences[[name]])))
  curr_sequence <- DNAStringSet(list(total_sequences[[name]]))
  
  hist_df <- lapply(dinucleotides, calculate_histogram, new_seqs = curr_sequence) %>%
    bind_rows() %>% 
  ...
  for (dnoi in dinucleotides){
    ...
    while(tries < (max_tries+1)){
      ...
      try(f <- fit_sine(
        hist_df %>% dplyr::filter(dinucleotide == dnoi, distance < max_dist),
        A_start = A_start,
        l_start = l_start,
        H_start = H_start)
      )
    ...
    # get the fit index with the lowest GOF
    ...
    hist_master_pred <- hist_df %>% 
      dplyr::filter(dinucleotide == dnoi, distance < max_dist) %>% 
      mutate(
        pred_alg = stats::predict(f, newdata = hist_df %>% dplyr::filter(dinucleotide == dnoi, distance < max_dist)))
    ...
    #ggsave(paste0(args[2], name, ".", dnoi, ".HISTCURVE", ".png"), plot=p)
    # output df
    list_df <- c(
      "sequence"=name,
      "dinucleotide"=dnoi,
    ...
    df <- df %>% bind_rows(list_df)
    hist_master <- hist_master %>% bind_rows(hist_master_pred)
  }
}
